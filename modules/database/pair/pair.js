...
var user_pair = (senderId, partnerId,collect,list) => {
    collect.deleteOne({ _id: partnerId.toString() }, (err, obj) => {
        let paired = list.collection('paired');
        console.log("User 1 : " + senderId.toString() + " ; User 2 : " + partnerId.toString() + "\n");
        var objinsert = [
            { id1: senderId.toString(), id2: partnerId.toString() },
            { id1: partnerId.toString(), id2: senderId.toString() }
        ]
...
var pair = (senderId, gender, fav) => {
    mongodb.connect(url, (err, database) => {
        ...
        collect.deleteOne({ _id: senderId.toString() }, (err, obj) => {
            if (err) throw err;
            if (gender === 'male') {
                if (fav === 'male') {
